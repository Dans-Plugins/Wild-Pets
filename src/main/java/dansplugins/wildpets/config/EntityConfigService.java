...
public class EntityConfigService {
    ...
    public EntityConfig acquireConfiguration(Entity entity) {
        for (EntityConfig entityConfig : entityConfigs) {
            if (entity.getType().name().equalsIgnoreCase(entityConfig.getType())) {
                if (wildPets.isDebugEnabled()) { System.out.println("[DEBUG] Configuration for " + entity.getType().name() + " found!"); }
                return entityConfig;
            }
        }
        if (wildPets.isDebugEnabled()) { System.out.println("[DEBUG] Configuration for " + entity.getType().name() + " not found! Using default configuration."); }
    ...
    public void initializeWithConfig() {
        if (wildPets.isDebugEnabled()) { System.out.println("[DEBUG] Initializing with config."); }

        Set<String> keys = wildPets.getConfig().getConfigurationSection("entityConfigurations").getKeys(false);

        for (String key : keys) {
            if (wildPets.isDebugEnabled()) { System.out.println("Looking at entity configuration for " + key); }
            HashMap<String, String> options = new HashMap<>();

            Set<String> configOptions = wildPets.getConfig().getConfigurationSection("entityConfigurations." + key).getKeys(false);

            for (String configOption : configOptions) {
                String value = wildPets.getConfig().getString("entityConfigurations." + key + "." + configOption);
                if (wildPets.isDebugEnabled()) { System.out.println("Looking at config option " + configOption + ". Value: " + value); }
