 ...
 */
public class LocalEntityConfigService {

    private static LocalEntityConfigService instance;

    private ArrayList<EntityConfig> entityConfigs = new ArrayList<>();

    private LocalEntityConfigService() {

    }

    public static LocalEntityConfigService getInstance() {
        if (instance == null) {
            instance = new LocalEntityConfigService();
        }
        return instance;
    }
    ...
    public void initializeWithConfig() {
        ...
        Set<String> keys = LocalConfigService.getInstance().getConfig().getConfigurationSection("entityConfigurations").getKeys(false);

        for (String key : keys) {
            if (WildPets.getInstance().isDebugEnabled()) { System.out.println("Looking at entity configuration for " + key); }
            HashMap<String, String> options = new HashMap<>();

            Set<String> configOptions = LocalConfigService.getInstance().getConfig().getConfigurationSection("entityConfigurations." + key).getKeys(false);

            for (String configOption : configOptions) {
                String value = LocalConfigService.getInstance().getConfig().getString("entityConfigurations." + key + "." + configOption);
